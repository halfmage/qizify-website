...
On independent benchmarks such as the [Artificial Analysis Intelligence Index](https://artificialanalysis.ai/models/open-source), the strongest open-weight models in mid-2026 come mostly from Chinese labs (Moonshot's Kimi, DeepSeek, Alibaba's Qwen, MiniMax, and Z.AI's GLM), with Mistral the leading European entry and Google's Gemma another Western option ([one mid-2026 roundup](https://officechai.com/ai/top-open-source-ai-models/) compiles the same index). For an EU buyer, that pattern is a reason to self-host rather than to avoid these models: running the open weights on your own EU infrastructure keeps your data in the EU regardless of where the model was trained. And if model provenance itself matters to you, Mistral is an EU-headquartered option under a clean licence.
